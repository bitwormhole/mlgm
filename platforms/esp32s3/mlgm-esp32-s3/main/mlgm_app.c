...
mlgm_error mlgm_app_init(mlgm_app *app);
...
mlgm_error mlgm_app_init(mlgm_app *app)
{
    ...
    // 初始化模块管理器
    mlgm_app_context *app_ctx = &app->context;
    mlgm_module_manager *mm = &app->modules;
    ...
    app_ctx->app = app;
    app_ctx->ble = mod_ble_1;
    app_ctx->modules = mm;
    app_ctx->rttest = mod_rttest_1;
    app_ctx->sd = mod_sd_1;
    app_ctx->udp = mod_udp_1;
    app_ctx->usb = mod_usb_1;
    app_ctx->wifi = mod_wifi_1;

    mlgm_module *mod_ble_2 = ble_comm_init(mod_ble_1, app_ctx);
    mlgm_module *mod_sd_2 = sd_card_init(mod_sd_1, app_ctx);
    mlgm_module *mod_usb_2 = usb_hid_init(mod_usb_1, app_ctx);
    mlgm_module *mod_wifi_2 = wifi_comm_init(mod_wifi_1, app_ctx);
    mlgm_module *mod_udp_2 = udp_debug_init(mod_udp_1, app_ctx);
    mlgm_module *mod_rttest_2 = runtime_test_init(mod_rttest_1, app_ctx);

    mod_usb_2->enabled = YES;
    mod_ble_2->enabled = NO;
    mod_wifi_2->enabled = NO;
    mod_rttest_2->enabled = YES;
    ...
    for (int i = 0; i < 99; i++)
    {
        mlgm_module *mod = mlist[i];
        if (mod)
        {
            mod->context = app_ctx;
            mlgm_module_manager_add(mm, mod);
        }
        else
        {
            break;
...
mlgm_error mlgm_app_on_create(mlgm_app *app)
{
    ...
    mlgm_module_manager *mm = &app->modules;
    mlgm_error err = mlgm_module_lifecycle_caller_init(&caller, mm);
    if (err)
    {
        return err;
    }
...
mlgm_error mlgm_app_on_start(mlgm_app *app)
{
    ...
    mlgm_module_lifecycle_caller caller;
    mlgm_module_manager *man = &app->modules;
